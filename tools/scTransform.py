# scTransform (Hafemeister, Satija, 2019) clone for Python from https://github.com/atarashansky/SCTransformPy/blob/main/SCTransform.py
...
def SCTransform(adata, layer=None, min_cells=5, gmean_eps=1, n_genes=2000, n_cells=None, bin_size=100, bw_adjust=3, inplace=True):
    ...
    for i in model_pars.columns:
        kr = statsmodels.nonparametric.kernel_regression.KernelReg(model_pars[i].values, genes_log_gmean_step1[:, None],
                                                                   ['c'], reg_type='ll', bw=[bw])
